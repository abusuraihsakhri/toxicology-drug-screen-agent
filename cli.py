"""
Command Line Interface for Toxicology Drug Screen Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="toxicology-drug-screen-agent", description="Toxicology Drug Screen Agent")
    ...
    if args.command == "audit":
        ...
        for _name, _val in [("primary", args.primary), ("secondary", args.secondary)]:
            if not isinstance(_val, (int, float)) or _val != _val:  # NaN check
                print(f"Error: --{_name} must be a finite number, got {_val!r}", file=sys.stderr)
                return 1
        ...
        print("=" * 80)
        print(f"  TOXICOLOGY DRUG SCREEN AGENT")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Toxicology Drug Screen Agent Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "batch":
        # Path traversal protection: validate input/output paths
        import os as _os
        _cwd = _os.getcwd()
    ...
    if args.command == "serve":
        ...
        print(f"Starting Toxicology Drug Screen Agent API server on http://{args.host}:{args.port}")
